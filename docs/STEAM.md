# Running Sitcord as a Steam Non-Steam Game

Sitcord is a small always-on-top, controller-driven overlay for
...
app's RPC connection once) before you launch Sitcord — it talks
...
| Platform | Artifacts |
| --- | --- |
| Windows | `Sitcord Setup <version>.exe` (NSIS installer) and `Sitcord <version>.exe` (portable, no install needed) |
| macOS | `Sitcord-<version>-<arch>.dmg`, containing `Sitcord.app` |
| Linux | `Sitcord-<version>-<arch>.AppImage` |

Run the installer/portable exe (Windows), copy `Sitcord.app` out
...
- Right-click (or Control-click) `Sitcord.app` → **Open** → confirm
...
3. Click **Browse...** and select the installed binary:
   - Windows: the installed `Sitcord.exe` (or the portable exe)
   - macOS: `Sitcord.app` (in `/Applications`)
...
- **LB + R3 together** (left bumper + right-stick click) shows/hides the window
  ...
  you've joined a channel and pop it open again to switch.

### Bringing the window back from inside a game

The LB+R3 show/hide chord works whenever Sitcord has input focus
...
- **System-tray / menu-bar icon** — Sitcord parks a microphone icon
...
- **Re-select it from Big Picture** — the most game-independent path, with no
  ...
  pinned Sitcord entry. The app only ever runs one copy, so a second
...
2. **Launch Sitcord first.** It connects, then you can tuck it away
...
- Select the pinned Sitcord entry → the running window comes to the
